...
export class Http extends EventEmitter implements X509ProvisioningTransport {
  ...
  cancel(callback: (err?: Error) => void): void {
    /* Codes_SRS_NODE_PROVISIONING_HTTP_18_041: [ `cancel` shall call `_restApiClient.abort` to cancel any operations that might be in progress. ] */
    if (this._restApiClient) {
      this._restApiClient.abort();
    }
    callback();
  }
